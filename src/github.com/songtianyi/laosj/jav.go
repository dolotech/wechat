...
func parseStar(item string, artist string) {
	...
	if len(m) < 2 {
		glog.Error("Find href error, %s", "len(m) < 2")
		return
	}
	url := m[1]
	...
	if len(artsrcs) != len(arts) {
		glog.Error(s.Url, len(artsrcs), len(arts))
		glog.Debug(artsrcs)
		glog.Debug(arts)
		panic(fmt.Errorf(""))
		return
	...
	for i, artsrc := range artsrcs {
		...
		if len(m1) < 2 {
			glog.Error("Find href fail, %s", artsrc)
			return
		}
		coverurl := m1[1]
		if *withImg == 0 {
			glog.Info(jvname, arts[i], artist, m1[1])
		} else {
			surl := strings.Split(coverurl, "/")
			filename := surl[len(surl)-1]
			glog.Info(jvname, arts[i], artist, filename)
		}
	}
}
