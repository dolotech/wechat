...
func (s *Downloader) Start() {
	...
	if err != nil {
		glog.Error("Start downloader fail %s", err)
		return
	}
	s.rc = rc
...
loop2:
	for {
		...
		case s.sema <- struct{}{}:
			...
			if !ok {
				// channel closed
				glog.Error("Channel s.urls may be closed")
			...
			go func() {
				if err := s.download(url.v); err != nil {
					// download fail
					// push back to redis
					glog.Error("Download %s fail, %s", url.v, err)
					if _, err := rc.RPush(s.SourceQueue, url.v); err != nil {
						glog.Error("Push back to redis failed, %s", err)
					}
				} else {
					// download success
					...
					if err := rc.HMSet(URL_CACHE_KEY, map[string]string{
						...
						glog.Error("Push to cache failed, %s", err)
					}
				}
			}()
		case <-tick:
			// print this every 2 seconds
			glog.Info("In queue: %d, doing: %d", len(s.urls), len(s.sema))
		}
	}
...
func (s *Downloader) download(url string) error {
	...
	if exist {
		// downloaded
		glog.Info("%s downloaded", url)
		return nil
	}

	glog.Info("Downloading %s", url)
	client := http.Client{
